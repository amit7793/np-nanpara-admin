    ...
    <div class="xl:px-[4rem] md:px-[20px] px-[10px] py-[2rem]">
        <div class="lg:flex  \ lg:flex-row lg:items-center justify-between gap-4 flex flex-col md:flex md:flex-row">
            ...
            <div class="px-[30px] mt-4 lg:flex">
                ...
                <form action="{{ route('upload.permission.pay.slip') }}" method="post" id="myForm" enctype="multipart/form-data">
                    @csrf
                    <div class="relative inline-block">
                        <button type="button" id="selectFileButton" class="bg-green-500 text-white ms-4 py-2 px-4 rounded"
                            onclick="document.getElementById('fileInput').click();">
                            Upload Pay Slip
                        </button>
                        <input type="file" id="fileInput" name="pay_slip" class="hidden" onchange="showFileName()"
                            required>
            ...
            <div class="lg:border border-[#B1B6C6] rounded-[20px]  lg:p-3 mt-10 flex flex-col  items-center justify-center">
                ...
                <div class="border border-[#B1B6C6] rounded-[20px] lg:w-[690px] md:w-[690px] w-full my-6  ">
                    <div>
                        ...
                        <div class="flex justify-between items-center px-[20px] py-[10px]   mt-4">
                            <p class=" font-medium text-[18px] leading-[30px] text-[#000000]">Fix charge</p>
                            <p class=" font-semibold text-[18px] leading-[30px] text-[#000000]">
                                {{ $charges['fixCharges'] }} ₹</p>
                        </div>
    ...
    function showApproveAlert(propertyId, status, charges) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                return $.ajax({
                    url: '{{ route('property.status.permission.change') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: propertyId,
