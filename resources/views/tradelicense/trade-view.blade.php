    ...
    <div class="border border-[#B1B6C6] rounded-[20px] mt-10    lg:px-[4rem] px-[10px]  ">
        <div class=" border-b border-[#B1B6C6] pb-[100px]">
            ...
            <div class="lg:flex lg:flex-row lg:items-center justify-between gap-4 flex flex-col md:flex md:flex-row">
                <h2 class=" font-bold text-[29px] lg:text-[32px] leading-[40px] text-left text-[#000000]">Trade License
                    Approval
                </h2>
                {{-- @if ($trades['check_status'] == 1)
                    @if ($trades['payment_status'] == 0)
                        <div class="px-[30px] mt-4 lg:flex">
                            <b>Pay Value</b>: <b>{{ $trades['amount'] ?? 0 }}</b> ₹
                            <form action="{{ route('upload.pay.slip') }}" method="post" id="myForm"
                                ...
                                <div class="relative inline-block">
                                    ...
                                    <input type="hidden" name="id" value="{{ $trades->id ?? 0 }}">
                                </div>
                ...
                return $.ajax({
                    ...
                    success: (response) => {
                        if (response.success) {
                            Swal.fire(
                                'Approved!',
                                'The trade license has been approved successfully.',
                                'success'
                            ).then(() => {
                                location
                            .reload();
